...
func ValidateTransaction(tx types.Transaction, head *types.Header, opts *ValidationOptions) error {
	...
	if tx.TxPreface().Value().Sign() < 0 {
		return ErrNegativeValue
	}
	// Ensure the transaction doesn't exceed the current block limit gas
	if (*head).GasLimit() < tx.TxPreface().GasLimit() {
		return ErrGasLimit
	}
	// Sanity check for extremely large numbers (supported by RLP or RPC)
	if tx.TxPreface().GasPrice().BitLen() > 256 {
		return ErrPriceVeryHigh
	}

	// Make sure the transaction is signed properly
	if _, err := tx.TxPreface().Validation().GetFrom(tx.TxPreface().TxHash()); err != nil {
	...
	if tx.TxPreface().GasLimit() < intrGas {
		return fmt.Errorf("%w: needed %v, allowed %v", ErrIntrinsicGas, intrGas, tx.TxPreface().GasLimit())
	}
	if tx.TxPreface().GasPrice().Cmp(opts.MinTip) < 0 {
		return fmt.Errorf("%w: tip needed %v, tip permitted %v", ErrUnderpriced, opts.MinTip, tx.TxPreface().GasPrice())
	}
	return nil
}
...
func ValidateTransactionWithState(tx types.Transaction, opts *ValidationOptionsWithState) error {
	...
	if next > tx.TxPreface().Nonce() {
		return fmt.Errorf("%w: next nonce %v, tx nonce %v", ErrNonceTooLow, next, tx.TxPreface().Nonce())
	}
	...
	if opts.FirstNonceGap != nil {
		if gap := opts.FirstNonceGap(from); gap < tx.TxPreface().Nonce() {
			return fmt.Errorf("%w: tx nonce %v, gapped nonce %v", ErrNonceTooHigh, tx.TxPreface().Nonce(), gap)
		}
	}
	...
	spent := opts.ExistingExpenditure(from, tx.TxPreface().Nonce())
	if prev := opts.ExistingCost(from, tx.TxPreface().Nonce()); prev != nil {
