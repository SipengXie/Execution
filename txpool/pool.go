...
func (pool *LegacyPool) SetGasTip(tip *big.Int) {
	...
	if tip.Cmp(old) > 0 {
		...
		for _, tx := range drop {
			pool.removeTx(tx.TxPreface().TxHash(), false)
		}
...
func (pool *LegacyPool) Pending() map[common.Address][]types.Transaction {
	pool.mu.Lock()
	defer pool.mu.Unlock()

	pending := make(map[common.Address][]types.Transaction, len(pool.pending))
	for addr, list := range pool.pending {
		txs := list.Ready(pool.pendingNonces.nonces[addr], pool.currentState.GetBalance(addr))
		for i, tx := range txs {
			if tx.TxPreface().GasPrice().Cmp(pool.gasTip.Load()) < 0 {
				txs = txs[:i]
				break
			}
		}
...
func (pool *LegacyPool) Status(hash common.Hash) TxStatus {
	...
	from := tx.TxPreface().From()

	pool.mu.RLock()
	defer pool.mu.RUnlock()

	if txList := pool.pending[from]; txList != nil && txList.txs.items[tx.TxPreface().Nonce()] != nil {
		return TxStatusPending
	} else if txList := pool.queue[from]; txList != nil && txList.txs.items[tx.TxPreface().Nonce()] != nil {
		...
		case <-evict.C:
			...
			for addr := range pool.queue {
				...
				if time.Since(pool.beats[addr]) > pool.config.Lifetime {
					...
					for _, tx := range list {
						pool.removeTx(tx.TxPreface().TxHash(), true)
					}
...
func (pool *LegacyPool) promoteTx(addr common.Address, hash common.Hash, tx types.Transaction) bool {
	...
	if old != nil {
		pool.all.Remove(old.TxPreface().TxHash())
		pool.priced.Removed(1)
	...
	pool.pendingNonces.Set(addr, tx.TxPreface().Nonce()+1)
...
//
// Note: In Ethereum, transactions are not marked as removed in the priced list because re-heaping
// is always explicitly triggered by SetBaseFee and it would be unnecessary and wasteful
// to trigger a re-heap is this function
//
// We will reheap the priced list if new head != nil
...
func (pool *LegacyPool) truncatePending() {
	...
	for pending > pool.config.GlobalSlots && !spammers.Empty() {
		...
		offender, _ := spammers.Pop() // pop the greatest priority one, which is the one with the most pending txs
		...
		if len(offenders) > 1 {
			...
			// pending > limited pending slots && last offender's pending length > threshold (initially this offender's pending length)
			for pending > pool.config.GlobalSlots && pool.pending[offenders[len(offenders)-2]].Len() > threshold {
				// for every already popped offender, reduce its pending txs by 1
				for i := 0; i < len(offenders)-1; i++ {
					list := pool.pending[offenders[i]]

					caps := list.Cap(list.Len() - 1) // means a kind of pop
					for _, tx := range caps {
						...
						pool.pendingNonces.SetIfLower(offenders[i], tx.TxPreface().Nonce())
		...
		for pending > pool.config.GlobalSlots && uint64(pool.pending[offenders[len(offenders)-1]].Len()) > pool.config.AccountSlots {
			for _, addr := range offenders {
				...
				for _, tx := range caps {
					...
					// Update the account nonce to the dropped transaction
					pool.pendingNonces.SetIfLower(addr, tx.TxPreface().Nonce())
					log.Trace("Removed fairness-exceeding pending transaction", "hash", hash)
				}
				pool.priced.Removed(len(caps))
				pendingGauge.Dec(int64(len(caps)))
...
func (pool *LegacyPool) truncateQueue() {
	...
	// delete from the account having the oldest heartbeat
	for drop := queued - pool.config.GlobalQueue; drop > 0 && len(addresses) > 0; {
		...
		for i := len(txs) - 1; i >= 0 && drop > 0; i-- {
			pool.removeTx(txs[i].TxPreface().TxHash(), true)
			drop--
...
func (pool *LegacyPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*SortedMap) {
	...
	if reset != nil {
		...
		for addr, list := range pool.pending {
			...
			nonces[addr] = highestPending.TxPreface().Nonce() + 1
		}
...
// scheduleReorgLoop schedules runs of reset and promoteExecutables.
...
func (pool *LegacyPool) reset(oldHead, newHead types.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash() {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			...
			if rem == nil {
				...
				// rewind old head to the same height to the new head
				...
				// rewind new head to the same height to the old head
				// may happen if the loop above is skipped
				...
				// if they are at the same height and not the same hash, rewind both
				// until they find a common ancestor
				// may be optimized by using LCA algorithm (log(n))
				// however, the height difference between old and new is within 64
				// so the LCA may not be useful. And we need to include every transaction...
				...
				reinject = types.TxDifference(discarded, included) // discared - (discarded & included)
			}
		}
	}
	...
	statedb, err := pool.chain.StateAt(newHead.Hash()) // here we get the state of the new head, but it's different from the ethereum implementation
	if err != nil {
	...
	// Ethereum needs to recover the from or some other fields
	// I am not sure if we need that
...
func (pool *LegacyPool) Add(txs types.Transactions, local bool, sync bool) []error {
...
func (pool *LegacyPool) addLocals(txs types.Transactions) []error {
...
func (pool *LegacyPool) addRemotes(txs types.Transactions) []error {
...
func (pool *LegacyPool) addRemotesSync(txs types.Transactions) []error {
...
func (pool *LegacyPool) addTxs(txs types.Transactions, local, sync bool) []error {
	...
	for i, tx := range txs {
		...
		if pool.all.Get(tx.TxPreface().TxHash()) != nil {
...
func (pool *LegacyPool) validateTxBasics(tx types.Transaction, local bool) error {
	...
	if err := ValidateTransaction(tx, pool.currentHead.Load(), opts); err != nil {
		return err
	}
	return nil
}
...
func (pool *LegacyPool) validateTx(tx types.Transaction, local bool) error {
	...
	if err := ValidateTransactionWithState(tx, opts); err != nil {
		return err
	}
	return nil
}
...
func (pool *LegacyPool) add(tx types.Transaction, local bool) (replaced bool, err error) {
	...
	// here valid Tx need to be implemented
	...
	from := tx.TxPreface().From()
	// from, err := tx.TxPreface().Validation().GetFrom(tx.TxPreface().TxHash())

	// If the transaction pool is full, discard underpriced transactions
	if uint64(pool.all.Slots()+numSlots(tx)) > pool.config.GlobalSlots+pool.config.GlobalQueue {
		// If the new transaction is underpriced, don't accept it
		if !isLocal && pool.priced.Underpriced(tx) {
			log.Trace("Discarding underpriced transaction", "hash", hash, "gasPrice", tx.TxPreface().GasPrice())
		...
		if !isLocal && pool.isGapped(from, tx) {
			...
			for _, dropTx := range drop {
				dropSender := dropTx.TxPreface().From()
				if list := pool.pending[dropSender]; list != nil && list.Contains(dropTx.TxPreface().Nonce()) {
		...
		for _, tx := range drop {
			log.Trace("Discarding freshly underpriced transaction", "hash", tx.TxPreface().TxHash(), "gasPrice", tx.TxPreface().GasPrice())
			underpricedTxMeter.Mark(1)
			dropped := pool.removeTx(tx.TxPreface().TxHash(), false)
			pool.changesSinceReorg += dropped
		}
	}

	// Try to replace an existing transaction in the pending pool
	if list := pool.pending[from]; list != nil && list.Contains(tx.TxPreface().Nonce()) {
...
func (pool *LegacyPool) isGapped(from common.Address, tx types.Transaction) bool {
	...
	for nonce := next; nonce < tx.TxPreface().Nonce(); nonce++ {
...
func (pool *LegacyPool) removeTx(hash common.Hash, outofbound bool) int {
	...
	if pending := pool.pending[addr]; pending != nil {
		if removed, invalids := pending.Remove(tx); removed {
			...
			for _, tx := range invalids {
				// Internal shuffle shouldn't touch the lookup set.
				pool.enqueueTx(tx.TxPreface().TxHash(), tx, false, false)
			}
